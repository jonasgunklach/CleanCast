...
struct FullPlayerView: View {
    ...
    var body: some View {
        GeometryReader { geometry in
            ZStack {
                ...
                ScrollView(.horizontal, showsIndicators: false) {
                    LazyHStack(spacing: 0) {
                        if SettingsManager.shared.debugModeEnabled {
                            ...
                            // Page 1: Ad Review
                            AdReviewView(accentColor: artworkColors?.accent)
                                .containerRelativeFrame(.horizontal)
                                .id(1)
                            
                            // Page 2: Player Controls
                            VStack(spacing: 30) {
                                controlsContent(geometry: geometry)
                            }
                            .containerRelativeFrame(.horizontal)
                            .id(2)
                ...
                .onChange(of: SettingsManager.shared.debugModeEnabled) { _, isDebug in
                    scrollPosition = isDebug ? 2 : 0
                }
                ...
                VStack {
                    Spacer()
                    HStack(spacing: 8) {
                        let totalDots = SettingsManager.shared.debugModeEnabled ? 4 : 2
                        
                        ForEach(0..<totalDots, id: \.self) { index in
                             Circle()
                                 .fill((scrollPosition ?? 0) == index ? Color.primary : Color.secondary.opacity(0.3))
    ...
    @ViewBuilder
    private func controlsContent(geometry: GeometryProxy) -> some View {
        // Draggable Top Area
        VStack(spacing: 30) {
            // Drag Handle
            Capsule()
